...
scene.add(light, spheres[0],groundMesh,skyMesh);
...
firebase.database().ref('sculptureData').once('value').then(function(snapshot) {
    ...
    for (var i=1; i<snapshot.numChildren(); i++){
        ...
        spheres[i]= new THREE.Mesh(pearlGeometry, pearlMaterial);
        spheres[i].position.set(snapshot.child(i).child("x").val(), snapshot.child(i).child("y").val(), snapshot.child(i).child("z").val());
        scene.add(spheres[i]);
    }
});

function writeSculptureData(obj) {
    firebase.database().ref('sculptureData').set(obj);
}
...
firebase.database().ref('sculptureData').on('value', function(snapshot) {
...
function draw() {
    ...
    if (toggle%2 == 0) {
        ...
        if (keyPress == 32 && collision[0].point.length() < 17) {
            spheres.push(new THREE.Mesh(pearlGeometry, pearlMaterial));
